...
export default function CommentPanel({itemId, type = "canteen/item"}) {
    const [user, setUser] = useState();
    const [messageApi, contextHolder] = message.useMessage();
    const [refresh, setRefresh] = useState(0);
    const [score, setScore] = useState(0);
    const {data, isLoading, error} = useSWR(
        `/api/rest/${type}/${itemId}/comment?${refresh}`,
    ...
    async function doCommentTopic(formData) {
        ...
        await fetchApiWithAuth(`/api/rest/${type}/${itemId}/comment`, {
            method: "POST",
    ...
    const doDeleteComment = (commentId) => {
        fetchApiWithAuth(`/api/rest/${type}/${itemId}/comment/${commentId}`, {
            method: "DELETE"
        }).then(r => {
            messageApi.open({
                type: 'success',
